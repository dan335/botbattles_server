...
public class Charger extends Ability {
    ...
    public void Fire() {
        ...
        if (isCharging) {
            ...
            new BlasterBullet(this, player.FirePositionX(), player.FirePositionY(), player.rotation, Math.min(80, time * sizeMultiplier), time * damageMultiplier, 1, color, 1.5, true);
            cooldown = defaultCooldown;
            lastFired = player.game.tickStartTime;
            SendCooldownMessage();
